...
class AsesorUPQViewSet(viewsets.ModelViewSet):
    serializer_class = AsesorUPQSerializer
    ...
    def create(self,request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'mensaje':'Asesor UPQ creado correctamente!'}, status = status.HTTP_201_CREATED)   
    ...
    def destroy(self, request, pk=None):
        ...
        if alumno:
            alumno.delete()
            return Response({'message':'Asesor UPQ eliminado correctamente'},status = status.HTTP_200_OK)
