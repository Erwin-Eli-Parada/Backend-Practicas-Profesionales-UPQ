...
class AsesorExternoViewSet(viewsets.ModelViewSet):
    serializer_class = AsesorExternoSerializer
    ...
    def create(self,request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'mensaje':'Asesor externo creado correctamente!'}, status = status.HTTP_201_CREATED)   
    ...
    def destroy(self, request, pk=None):
        ...
        if alumno:
            alumno.delete()
            return Response({'message':'Asesor externo eliminado correctamente'},status = status.HTTP_200_OK)
