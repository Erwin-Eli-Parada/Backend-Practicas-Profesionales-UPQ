...
class ComentarioViewSet(viewsets.ModelViewSet):
    ...
    def create(self,request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'mensaje':'Comentario creado correctamente!'}, status = status.HTTP_201_CREATED)   
    ...
    def destroy(self, request, pk=None):
        ...
        if alumno:
            alumno.delete()
            return Response({'message':'Comentario eliminada correctamente'},status = status.HTTP_200_OK)
        return Response({'error':'No existe'},status = status.HTTP_400_BAD_REQUEST)
    
@api_view(['GET'])
def comentario_detail_view(request, pk=None):
    if request.method == 'GET':
        comentario = Comentario.objects.filter(id_alumno = pk).all()
        if comentario==None:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        comentario_serializer = ComentarioSerializer(comentario, many=True)
